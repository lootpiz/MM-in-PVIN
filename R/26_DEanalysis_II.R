...
roi <- roiL[[section]]
roi$cluster <- factor(cls)

if (all(colnames(roi) == names(cls))) {
        pvalues <- wilcoxauc(roi, 'cluster')
        pvalues <- pvalues[which(pvalues$group == 1),] # 1 vs. the others
        ...
        write.table(df, file.path(outDir, paste0("Wilcoxon_", section, "_cl1.txt")), row.names=F, col.names=T, quote=F, sep="\t")
        ...
        pdf(file.path(outDir, paste0("Wilcoxon_", section, "_cl1.pdf")), width=8, height=6)
        print(volcanoPlot)
        dev.off()
}
